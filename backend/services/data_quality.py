...
def detect_outliers(
    df: pd.DataFrame, numeric_cols: List[str] = None, iqr_multiplier: float = 1.5
) -> Dict:
...
def compute_data_completeness(
    df: pd.DataFrame, critical_cols: List[str] = None
) -> Dict:
    ...
    if critical_cols:
        ...
        if critical_present:
            critical_complete = (
                df[critical_present].notna().sum().sum()
                / (len(df) * len(critical_present))
            ) * 100
...
def compute_sensor_health(
    df: pd.DataFrame, feature_groups: Dict[str, List[str]] = None
) -> pd.DataFrame:
    ...
    if feature_groups is None:
        feature_groups = {
            "All Data": df.select_dtypes(include=[np.number]).columns.tolist()
        }
...
"""
    ...
    for idx, row in health.iterrows():
        status = (
            "🟢 Good"
            if row["Health Score (0-100)"] >= 80
            else "🟡 Fair"
            if row["Health Score (0-100)"] >= 60
            else "🔴 Poor"
        )
        summary += (
